...
DBNAMES="desig6_blog desig6_ddmsb desig6_ecom desig6_knbDB desig6_paypal desig6_photo"
...
dbdump () {
/opt/mysql5/bin/mysqldump --user=$USERNAME --password=$PASSWORD --host=$DBHOST $OPT $1 > $2
return 0
}
...
echo ======================================================================
	...
	if [ $DNOW = $DOWEEKLY ]; then
		...
		eval rm -f "$BACKUPDIR/weekly/$DB_week.$REMW.*" 
		echo
	...
	# Daily Backup
		...
		eval rm -f "$BACKUPDIR/daily/$DB/*.$DOW.sql.*" 
		echo
...
echo ======================================================================
	...
	# Daily Backup
		...
		echo
		echo Rotating last weeks Backup...
		eval rm -f "$BACKUPDIR/daily/*.$DOW.sql.*" 
		echo
